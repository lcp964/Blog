 ...
 let isMobileFlag =false, //标记为flase
   ...
   function fn_check_mobile() {
        ...
        $.ajax({
            ...
            .done(function (res) {
                if (res.data.count !== 1) {
                    message.showError('【' + res.data.mobile + '】' + '手机号没有注册,请重新注册！');


                    // alert('手机号已经注册,请重新输入！')
                } else {

                    message.showSuccess('【' + res.data.mobile + '】' + '执行发送短信');
    ...
    //短信发送
     ...
     // 5、注册逻辑
                ...
                $register.submit(function (e) {
                    ...
                    let sPassword = $("input[name=ne_password]").val();
                    // console.log(sPassword);

                    let sPasswordRepeat = $("input[name=for_password]").val();
                    // console.log(sPassword);
                   ...
                   // alert(SdataParams);
                    ...
                    $.ajax({
                        ...
                        .done(function (res) {
                            if (res.errno === "0") {
                                // 注册成功
                                message.showSuccess('恭喜你，忘记密码修改成功');
                                setTimeout(() => {
                                    // 注册成功之后重定向到主页
